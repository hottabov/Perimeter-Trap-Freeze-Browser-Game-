...
export class Sfx {
  ...
  sparx() {
    if (!this.A) return;
    this.tone({ f: 1800, glideTo: 300, type: 'sawtooth', dur: 0.35, gain: 0.07, cutoff: 4000, send: 0.3 });
    this.noise({ dur: 0.25, gain: 0.08, type: 'bandpass', f: 3000, q: 6, send: 0.2 });
  }
  powerSpawn() { if (this.A) [0, 2, 4].forEach((d, k) => this.tone({ f: this.note(d, 4), type: 'sine', dur: 0.4, gain: 0.04, at: k * 0.06, send: 0.8 })); }
  powerup(type) {
    if (!this.A) return;
    const steps = type === 'slow' ? [4, 2, 0, -2] : [0, 2, 4, 7];
    steps.forEach((d, k) => this.tone({ f: this.note(d, 3), type: 'triangle', dur: 0.3, gain: 0.08, at: k * 0.055, send: 0.6 }));
    if (type === 'life') this.tone({ f: this.note(7, 2), type: 'sine', dur: 1.2, gain: 0.08, at: 0.25, attack: 0.05, send: 0.8 });
  }
  shieldBreak() {
    if (!this.A) return;
    this.tone({ f: 900, glideTo: 180, type: 'square', dur: 0.3, gain: 0.08, cutoff: 2500, send: 0.4 });
    this.noise({ dur: 0.35, gain: 0.2, type: 'highpass', f: 2500, send: 0.3 });
  }
  bossHit() {
    if (!this.A) return;
    this.tone({ f: 70, glideTo: 30, type: 'sine', dur: 0.7, gain: 0.8, send: 0.1 });
    this.noise({ dur: 0.5, gain: 0.4, type: 'lowpass', f: 900, to: 150, send: 0.3 });
  }
  bossBreak() {
    if (!this.A) return;
  ...
  goal() {
    if (!this.A) return;
    [0, 2, 4, 7, 9].forEach((d, k) => this.tone({ f: this.note(d, 2), type: this.A.wave, dur: 0.7, gain: 0.07, at: k * 0.09, cutoff: 3000, send: 0.7 }));
  }
  levelIntro() {
    if (!this.A) return;
    this.noise({ dur: 1.4, gain: 0.08, type: 'bandpass', f: 200, to: 2400, q: 0.6, attack: 0.6, send: 0.6 });
    this.tone({ f: this.note(0, 1), type: 'sine', dur: 2, gain: 0.1, attack: 0.4, send: 0.7 });
  }
  sinkOut() { if (this.A) this.noise({ dur: 0.9, gain: 0.1, type: 'lowpass', f: 2400, to: 120, attack: 0.1, send: 0.5 }); }
